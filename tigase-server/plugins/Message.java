...
//			log.log(Level.INFO, "1 {0}", new Object[]{session});
			return;
...
//						log.severe("Message from sender to sender: " + packet.toString());
				...
				if (conns.size() > 0) {
                    ...
                    if(packet.getType() == StanzaType.chat) {
                        ...
                        // save to history
                        //
                        QBChatServices.getInstance().savePrivateChatMessageToHistory(packet.getElement(), false, null);
                    }
                    //
                    //
